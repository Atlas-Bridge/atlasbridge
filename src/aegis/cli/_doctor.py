...
def _check_config() -> dict:
    try:
        from aegis.core.config import aegis_dir, load_config

        cfg_path = aegis_dir() / "config.toml"
        if not cfg_path.exists():
            return {
                "name": "Config file",
                "status": "warn",
                "detail": f"not found at {cfg_path} — run: aegis setup",
            }
    ...
    try:
        ...
        if not token:
            return {
                "name": "Bot token",
                "status": "warn",
                "detail": "not configured — run: aegis setup",
            }
...
def _check_systemd() -> dict | None:
    ...
    from aegis.os.systemd.service import is_systemd_available
...
def _check_systemd_service() -> dict | None:
    ...
    if not sys.platform.startswith("linux"):
        return None
    from aegis.os.systemd.service import systemd_user_dir
    ...
    return {
        "name": "aegis.service",
        "status": "warn",
        "detail": "not installed — run: aegis setup --install-service",
    }
